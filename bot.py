...
async def safe_answer_photo(message: types.Message, photo: str, caption: str, reply_markup=None, parse_mode=ParseMode.MARKDOWN):
    """Отправляет фото, а если не получилось — отправляет запасную картинку или текст"""
    try:
        ...
        try:
            # Пробуем отправить запасную картинку
            await message.answer_photo(
                photo=IMG_FALLBACK,
                caption=caption,
                parse_mode=parse_mode,
            ...
            logging.info(f"Отправлена запасная картинка вместо {photo}")
            return True
        except Exception as e2:
            logging.warning(f"Не удалось отправить запасную картинку: {e2}")
            # Если и запасная не работает — отправляем просто текст
