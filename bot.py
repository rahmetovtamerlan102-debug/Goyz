...
def buy_premium_choose_menu():
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Telegram Stars", callback_data="buy_premium_stars_choose")],
        [InlineKeyboardButton(text="CryptoBot (USDT)", callback_data="buy_premium_usd_choose")],
        [InlineKeyboardButton(text="xRocket (USDT)", callback_data="buy_premium_xrocket_choose")],
...
# ========== ФУНКЦИЯ СОЗДАНИЯ ПЛАТЕЖА В XROCKET ==========

async def create_xrocket_invoice(user_id: int, days: int, amount: float):
    """Создаёт счёт в xRocket и возвращает ссылку для оплаты."""
    if not XROCKET_API_KEY:
        logging.warning("XROCKET_API_KEY не задан")
        return None

    client = xRocketClient(api_key=XROCKET_API_KEY)
    ...
    try:
        ...
        invoice = await client.create_invoice(
            ...
            payload=f'premium_{user_id}_{days}'  # Передаём данные для вебхука
        )
        
        # invoice.link - это ссылка для оплаты
        return invoice.link
    except Exception as e:
...
async def xrocket_webhook(request):
    """Обработчик вебхука от xRocket Pay"""
    try:
        raw_body = await request.read()
        ...
        if invoice_id:
            ...
            if not is_new:
                logging.info(f"Инвойс xRocket {invoice_id} уже обработан, пропускаем")
                return web.Response(text="OK", status=200)
