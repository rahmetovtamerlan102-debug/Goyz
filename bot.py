...
# ========== СОСТОЯНИЯ ==========
admin_states = {}          # Для админ-действий
waiting_for_words = set()  # Для ожидания ввода слов в фильтры

# ========== ОЧИСТКА СОСТОЯНИЯ ==========

def clear_waiting_state(user_id: int):
    """Очищает состояние ожидания ввода слов для пользователя"""
...
async def init_db():
    ...
    # Таблица обработанных платежей (идемпотентность) - с TEXT полем
    await conn.execute('''
...
async def mark_invoice_processed(payment_id: str) -> bool:
    ...
    Возвращает True, если платёж обработан впервые.
    Возвращает False, если платёж уже был обработан.
    """
    if not payment_id:
        return True
...
async def extend_premium(user_id: int, days: int) -> datetime:
    """
    Атомарно продлевает Premium пользователю.
    Возвращает новую дату окончания Premium.
    """
    logging.info(f"Продление Premium для {user_id} на {days} дней")
    
    conn = await asyncpg.connect(DATABASE_URL)
    try:
        # Используем умножение интервала (без конкатенации строк)
        ...
        ''', days, user_id)  # days передаётся как int
...
async def safe_answer_photo(message: types.Message, photo: str, caption: str, reply_markup=None, parse_mode=None):
    ...
    Если parse_mode не указан, используется глобальная настройка бота (HTML).
    """
    try:
        # Используем parse_mode только если он явно передан
        kwargs = {
            "photo": photo,
            "caption": caption,
...
async def crypto_webhook(request):
    """Обработчик вебхука от CryptoBot с проверкой подписи и идемпотентностью"""
    try:
        ...
        if payload and payload.startswith("premium_"):
            ...
            # Атомарное продление Premium (без гонок)
...
async def start_command(message: Message):
    ...
    # Сбрасываем незавершённый ввод слов
...
async def universal_message_handler(message: Message):
    ...
    # Игнорируем сообщения без текста (стикеры, фото, голосовые)
    if not message.text:
        return
    ...
    # 1. Админские состояния (рассылка / ввод ID)
    ...
    # 2. Ожидание ввода слов для фильтров
    if user_id in waiting_for_words:
        await handle_words_input(message)
        return
    
    # 3. Если сообщение не обработано — показываем меню
    await safe_answer_photo(
        message,
        IMG_MENU,
        "<b>Добро пожаловать в Hunter!</b>\n\nВыберите действие:",
...
async def admin_input_handler(message: Message):
    """Обработчик ввода админа (ID, количество)"""
    user_id = message.from_user.id
...
async def handle_words_input(message: Message):
    """Обработчик ввода слов для фильтров"""
...
async def pre_checkout_handler(pre_checkout_query: PreCheckoutQuery):
    """Обработчик pre_checkout для Stars"""
...
async def filter_words_callback(callback: CallbackQuery):
    ...
    # Добавляем пользователя в список ожидающих ввода слов
