...
async def filter_start_callback(callback: CallbackQuery):
    ...
    try:
        ...
        # Удаляем старое сообщение с фильтрами и запускаем поиск
        await callback.message.delete()
        await start_search_with_animation(callback.message, user_id, filters, is_filter=True)
    except Exception as e:
...
async def chars_letters_callback(callback: CallbackQuery):
    ...
    # Удаляем сообщение с выбором символов и запускаем поиск
    await callback.message.delete()
...
async def magnet_action_callback(callback: CallbackQuery):
    ...
    try:
        ...
        # Удаляем сообщение с выбором магнита и запускаем поиск
        await callback.message.delete()
        await start_search_with_animation(callback.message, user_id, filters, is_magnet=True)
    except Exception as e:
