...
user_data = {}
filter_data = {}
search_status = {}  # Для отслеживания поиска
...
async def filter_start_callback(callback: CallbackQuery):
    ...
    # Запускаем поиск с отображением процесса
...
async def magnet_action_callback(callback: CallbackQuery):
    ...
    # Запускаем поиск с анимацией для магнита
    user_id = callback.from_user.id
    filters = filter_data.get(user_id, get_default_filters())
    filters["length"] = 6  # Магнит ищет 6 символов
    await start_search_with_animation(callback.message, user_id, filters)
...
# ========== ОСНОВНЫЕ ФУНКЦИИ ПОИСКА С АНИМАЦИЕЙ ==========
...
async def start_search_with_animation(message: types.Message, user_id: int, filters: dict):
    """Запускает поиск с анимацией проверки юзернеймов"""
    if user_id not in user_data:
        user_data[user_id] = {
            "attempts": 5,
            "invited": 0,
    ...
    # Генерируем и проверяем юзернеймы
    found_username = None
    attempts = 0
    max_attempts = 50 if is_premium else 20
    
    # Список для отображения проверяемых юзернеймов
    checking_names = []
    ...
    while attempts < max_attempts:
        ...
        checking_names.append(username)
        if len(checking_names) > 3:
            checking_names.pop(0)
        
        # Обновляем сообщение с текущим проверяемым юзернеймом
...
async def do_search_with_animation(message: types.Message, user_id: int, length: int, chars_type: str, chars_name: str):
    """Поиск с анимацией для 5 и 6 символов"""
    if user_id not in user_data:
        user_data[user_id] = {
            "attempts": 5,
            "invited": 0,
