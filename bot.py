...
async def search_5_callback(callback: CallbackQuery):
    ...
    await show_chars_selection(callback, user_id, 5)  # Передаём callback, а не message

@dp.callback_query(lambda c: c.data == "search_6")
async def search_6_callback(callback: CallbackQuery):
    user_id = callback.from_user.id
    await show_chars_selection(callback, user_id, 6)  # Передаём callback, а не message
...
async def show_chars_selection(callback: CallbackQuery, user_id: int, length: int):
    """Показывает выбор символов, удаляя старое сообщение с фото"""
    if user_id not in user_data:
        user_data[user_id] = {
            "attempts": 5,
            "invited": 0,
