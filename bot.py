...
# ========== НАСТРОЙКИ ==========
BOT_TOKEN = os.getenv("BOT_TOKEN")  # Для Render: переменная окружения
# BOT_TOKEN = "ВАШ_ТОКЕН"  # Для локального запуска (раскомментировать)
...
# ========== ФУНКЦИЯ ПРОВЕРКИ ЮЗЕРНЕЙМА (Telegram + Fragment) ==========

def check_username(username: str) -> dict:
    """
    Проверяет доступность юзернейма на t.me и fragment.com
    ...
    # 1. ПРОВЕРКА НА TELEGRAM (t.me)
    try:
        tg_resp = requests.get(f"https://t.me/{username}", headers=headers, timeout=10)
        # Если в ответе есть фраза "If you have Telegram, you can contact" - профиль существует
        if "If you have Telegram, you can contact" not in tg_resp.text:
            result["telegram"] = True  # Свободен
        else:
            result["telegram"] = False  # Занят
...
def generate_username(length: int, chars_type: str = "letters") -> str:
    """Генерирует случайный юзернейм заданной длины"""
...
async def search_5_callback(callback: CallbackQuery):
    ...
    await show_chars_selection(callback.message, user_id, 5)

@dp.callback_query(lambda c: c.data == "search_6")
async def search_6_callback(callback: CallbackQuery):
    user_id = callback.from_user.id
    await show_chars_selection(callback.message, user_id, 6)
...
async def info_callback(callback: CallbackQuery):
    await callback.message.edit_text(
        "ИНФОРМАЦИЯ\n\n"
        "Catchu - бот для поиска свободных юзернеймов\n\n"
...
async def show_chars_selection(message: types.Message, user_id: int, length: int):
    if user_id not in user_data:
        user_data[user_id] = {
            "attempts": 5,
            "invited": 0,
...
async def do_search(message: types.Message, user_id: int, length: int, chars_type: str, chars_name: str):
    ...
    # Ищем свободный юзернейм (максимум 20 попыток)
    while attempts < 20:
        username = generate_username(length, chars_type)
        result = check_username(username)  # Проверка на Telegram + Fragment
        attempts += 1
        if result["available"]:
            found_username = username
            break
    
    # Уменьшаем попытки пользователя
    ...
    if found_username:
        ...
        # Формируем результат с проверкой на Telegram и Fragment
        text = (
            "НАЙДЕН\n\n"
            "ЮЗЕРНЕЙМ НАЙДЕН\n"
