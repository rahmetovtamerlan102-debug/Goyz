...
REQUIRED_CHANNEL = "@huntergoaj"  # Канал для подписки (сделайте его публичным или добавьте бота админом)
...
async def check_subscription(user_id: int) -> bool:
    """Проверяет подписку на канал, если канал приватный - пропускаем проверку"""
    try:
        chat_member = await bot.get_chat_member(chat_id=REQUIRED_CHANNEL, user_id=user_id)
        return chat_member.status in ["member", "administrator", "creator"]
    except Exception as e:
        # Если бот не может проверить подписку (канал приватный или бот не админ) - пропускаем
        logging.warning(f"Не удалось проверить подписку: {e}")
        return True  # Пропускаем проверку, чтобы бот работал
