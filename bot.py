...
# ========== ХРАНИЛИЩЕ ДАННЫХ ==========
user_data = {}
filter_data = {}  # Хранилище настроек фильтров для каждого пользователя
...
async def filter_start_callback(callback: CallbackQuery):
    ...
    # Генерация юзернейма по настройкам фильтров
    found_username = None
    attempts = 0
    ...
    while attempts < 20:
        if filters["words"]:
            ...
            if len(username) < filters["length"]:
                username = generate_username(filters["length"], filters["chars_type"])
        else:
            username = generate_username(filters["length"], filters["chars_type"])
