...
XROCKET_WEBHOOK_TOKEN = os.getenv("XROCKET_WEBHOOK_TOKEN")
...
def buy_premium_choose_menu():
    ...
    # Всегда показываем Stars
    keyboard.append([InlineKeyboardButton(text="Telegram Stars", callback_data="buy_premium_stars_choose")])
    
    # CryptoBot, если есть токен
    if CRYPTO_TOKEN:
        keyboard.append([InlineKeyboardButton(text="CryptoBot (USDT)", callback_data="buy_premium_usd_choose")])
    
    # xRocket, только если есть API-ключ
    if XROCKET_API_KEY and len(XROCKET_API_KEY) > 10:
        keyboard.append([InlineKeyboardButton(text="xRocket (USDT)", callback_data="buy_premium_xrocket_choose")])
...
def buy_premium_usd_menu():
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="3 дня - 3 USD", callback_data="usd_3")],
        [InlineKeyboardButton(text="7 дней - 5 USD", callback_data="usd_7")],
        [InlineKeyboardButton(text="30 дней - 10 USD", callback_data="usd_30")],
        [InlineKeyboardButton(text="Назад", callback_data="buy_premium_choose")]
    ])
    return keyboard

def buy_premium_xrocket_menu():
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="1 день - 1 USDT", callback_data="xrocket_1")],
        [InlineKeyboardButton(text="3 дня - 3 USDT", callback_data="xrocket_3")],
        [InlineKeyboardButton(text="7 дней - 5 USDT", callback_data="xrocket_7")],
        [InlineKeyboardButton(text="30 дней - 10 USDT", callback_data="xrocket_30")],
        [InlineKeyboardButton(text="Назад", callback_data="buy_premium_choose")]
    ])
    return keyboard
...
def verify_crypto_signature(body: bytes, signature: str) -> bool:
    if not CRYPTO_TOKEN:
        return False
...
async def create_crypto_invoice(user_id: int, days: int, amount: float):
    if not CRYPTO_TOKEN:
        logging.warning("CRYPTO_TOKEN не задан")
        return None
    ...
    try:
        ...
        if data.get("ok"):
            ...
            logging.error(f"Ошибка CryptoBot: {error}, полный ответ: {data}")
            return None
    except Exception as e:
        logging.exception("Ошибка при создании инвойса в CryptoBot")
        return None

# ========== ФУНКЦИЯ СОЗДАНИЯ ПЛАТЕЖА В XROCKET (API v2) ==========

async def create_xrocket_invoice(user_id: int, days: int, amount: float):
    """Создаёт счёт в xRocket Pay API v2."""
    if not XROCKET_API_KEY:
        logging.warning("XROCKET_API_KEY не задан")
        return None
    
    url = "https://api.xrocket.io/v2/invoices"
    headers = {
    ...
    data = {
        ...
        "redirect_url": f"https://t.me/{BOT_NAME}",
        "webhook_url": f"https://goyz-6w27.onrender.com/xrocket_webhook"
    }
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, json=data) as response:
                ...
                logging.info(f"Ответ xRocket: {result}")
                
                if response.status == 201 and result.get("data", {}).get("checkout_url"):
                    return result["data"]["checkout_url"]
                else:
                    error = result.get("error", {}).get("message", "Неизвестная ошибка")
                    logging.error(f"Ошибка xRocket: {error}")
                    return None
    except Exception as e:
        logging.exception("Ошибка при создании инвойса в xRocket")
        return None

# ========== HEALTH CHECK + ВЕБХУКИ ==========
...
async def crypto_webhook(request):
    try:
        ...
        if not verify_crypto_signature(raw_body, signature):
            logging.warning("Неверная подпись вебхука CryptoBot")
        ...
        if invoice_id:
            is_new = await mark_invoice_processed(invoice_id)
            if not is_new:
                logging.info(f"Инвойс {invoice_id} уже обработан, пропускаем")
                return web.Response(text="OK", status=200)
        else:
            logging.warning("Получен вебхук без invoice_id")
        ...
        if payload and payload.startswith("premium_"):
            ...
            if new_until:
                await log_admin_action(
                    user_id,
                    "buy_premium_usd",
                    details=f"Куплен Premium на {days} дней через CryptoBot (USD). Инвойс: {invoice_id}"
                )
...
async def xrocket_webhook(request):
    """Обработчик вебхука от xRocket Pay API v2."""
    try:
        raw_body = await request.read()
        signature = request.headers.get("X-Rocket-Signature", "")
        
        if not XROCKET_WEBHOOK_TOKEN:
            logging.warning("XROCKET_WEBHOOK_TOKEN не задан")
            return web.Response(text="ERROR", status=500)
        
        # Проверка подписи
        computed_signature = hmac.new(
            XROCKET_WEBHOOK_TOKEN.encode(),
            raw_body,
            hashlib.sha256
        ).hexdigest()
        
        if not hmac.compare_digest(computed_signature, signature):
            logging.warning("Неверная подпись вебхука xRocket")
            return web.Response(text="FORBIDDEN", status=403)
        ...
        payload_str = invoice_data.get("payload", "{}")
        ...
        if invoice_id:
            is_new = await mark_invoice_processed(f"xrocket_{invoice_id}")
            if not is_new:
                logging.info(f"Инвойс xRocket {invoice_id} уже обработан")
                return web.Response(text="OK", status=200)
        ...
        if user_id:
            ...
            if new_until:
                await log_admin_action(
                    user_id,
                    "buy_premium_xrocket",
                    details=f"Куплен Premium на {days} дней через xRocket (USDT). Инвойс: {invoice_id}"
                )
        ...
        logging.exception("Ошибка при обработке вебхука от xRocket")
        return web.Response(text="ERROR", status=500)
...
async def start_web_server():
    ...
    app.router.add_post('/xrocket_webhook', xrocket_webhook)
...
# ========== АДМИН-ОБРАБОТЧИКИ (БЕЗ ДЕКОРАТОРОВ) ==========
...
@dp.callback_query(lambda c: c.data == "buy_premium_xrocket_choose")
async def buy_premium_xrocket_choose_callback(callback: CallbackQuery):
    await callback.message.delete()
    await callback.message.answer(
        "Выберите срок подписки:",
        reply_markup=buy_premium_xrocket_menu()
    )
    await callback.answer()
...
async def usd_payment_callback(callback: CallbackQuery):
    ...
    if not invoice_url:
        await callback.answer(
            "Ошибка при создании платежа. Минимальная сумма 1 USDT.\nПопробуйте выбрать другой тариф или оплатить в Stars.",
            show_alert=True
        )
        return
...
@dp.callback_query(lambda c: c.data.startswith("xrocket_"))
async def xrocket_payment_callback(callback: CallbackQuery):
    user_id = callback.from_user.id
    days = int(callback.data.replace("xrocket_", ""))
    ...
    if not XROCKET_API_KEY:
        await callback.answer(
            "xRocket временно недоступен. Попробуйте другой способ оплаты.",
            show_alert=True
        )
        return
    
    invoice_url = await create_xrocket_invoice(user_id, days, amount)
    
    if not invoice_url:
        await callback.answer(
            "Ошибка при создании платежа. Попробуйте другой способ.",
            show_alert=True
        )
        return
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Оплатить", url=invoice_url)],
        [InlineKeyboardButton(text="Назад", callback_data="buy_premium_xrocket_choose")]
    ])
...
# ========== PRE_CHECKOUT И SUCCESSFUL_PAYMENT ==========
...
async def successful_payment(message: Message):
    ...
    is_new = await mark_invoice_processed(charge_id)
    if not is_new:
        logging.info(f"Платёж {charge_id} уже обработан, пропускаем")
        return
    ...
    if new_until:
        await log_admin_action(
            user_id,
            "buy_premium_stars",
            details=f"Premium {days} дней за Stars, charge_id: {charge_id}"
        )
        await message.answer(
            f"Premium активирован на {days} дней!\n\n"
            f"Действует до: {new_until.strftime('%d.%m.%Y %H:%M')}\n\n"
...
async def info_callback(callback: CallbackQuery):
    try:
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="Пользовательское соглашение", url="https://telegra.ph/Polzovatelskoe-soglashenie-08-27-60")],
            [InlineKeyboardButton(text="Политика конфиденциальности", url="https://telegra.ph/Politika-konfidencialnosti-08-27-84")],
            [InlineKeyboardButton(text="Официальный канал", url=f"https://t.me/{REQUIRED_CHANNEL.replace('@', '')}")],
            [InlineKeyboardButton(text="В меню", callback_data="menu")]
        ])
